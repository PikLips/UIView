...
- (void)setUp
{
    [super setUp];
        ...
        testAppDelegate = (UIVAppDelegate *)[[UIApplication sharedApplication] delegate];  //PikLips: creates a separate instance for testing of the same Class as AppDelegate which includes UIGestureRecognizer delegate
...
- (void)tearDown
{
    ...
    -(void) test2_TheWindow
    {
        ...
        STAssertEquals(testAppDelegate.window.bounds.size, testAppDelegate.window.screen.applicationFrame.size, @"Window may not be best sized for its screen, usually not a problem but check status bar."); //PikLips: default
